...
export default function ClientPOS() {
  ...
  const handleSearchCustomer = async () => {
    ...
    try {
      const q = query(collection(db, "sales"), where("customerPhone", "==", customerPhone), limit(1));
      const querySnapshot = await getDocs(q);
      
      if (!querySnapshot.empty) {
        const pastSale = querySnapshot.docs[0].data();
        setCustomerName(pastSale.customerName || '');
        setCustomerStatus('returning');
        setCustomerDB(prev => [...prev, { phone: customerPhone, name: pastSale.customerName }]);
      } else {
        ...
        <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '8px', border: '1px solid #ddd', marginBottom: '20px', display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '200px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button type="button" onClick={handleSearchCustomer} style={{ padding: '10px 15px', backgroundColor: '#2c3e50', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
                Search
              </button>
            </div>
          </div>
