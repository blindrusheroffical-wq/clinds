...
export default function BillingPOS() {
  ...
  const handleSearchCustomer = async () => {
    ...
    try {
      const q = query(collection(db, "sales"), where("customerPhone", "==", customerPhone));
      const querySnapshot = await getDocs(q);
      if (!querySnapshot.empty) {
        const sales = querySnapshot.docs.map(d => d.data());
        sales.sort((a, b) => (b.date?.seconds || 0) - (a.date?.seconds || 0)); // Latest first
        const pastSale = sales[0];
        setCustomerName(pastSale.customerName || '');
        setCustomerStatus('returning');
        if (pastSale.total) setLastPurchaseAmt(pastSale.total.toFixed(2));
        if (pastSale.date?.toDate) setLastPurchaseDate(pastSale.date.toDate().toLocaleDateString());
      } else {
    ...
    setTimeout(async () => {
      ...
      if (window.confirm("Bill printed! Do you want to save this to the database and clear the cart?")) {
        ...
        if (customerPhone.trim() && customerName.trim()) {
          ...
          if (!existing) {
            setCustomerDB(prev => [...prev, { phone: customerPhone.trim(), name: customerName.trim() }]);
          }
        }
  ...
  const handleOpenReport = async () => {
    ...
    try {
      const startOfDay = new Date();
      startOfDay.setHours(0, 0, 0, 0); // Set time to today midnight
      const q = query(collection(db, "sales"), where("date", ">=", startOfDay));
      const querySnapshot = await getDocs(q);
      const fetchedSales = querySnapshot.docs.map(doc => doc.data());
      fetchedSales.sort((a, b) => (b.date?.seconds || 0) - (a.date?.seconds || 0));
      setDailySales(fetchedSales);
    } catch (error) {
      console.error("Error fetching daily sales:", error);
    }
    ...
    <>
        ...
        <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '200px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              />
          <button type="button" onClick={handleSearchCustomer} disabled={isSearching} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '12px 20px', backgroundColor: '#222', color: '#D4AF37', border: 'none', borderRadius: '5px', cursor: isSearching ? 'not-allowed' : 'pointer', fontWeight: 'bold' }}>
            {isSearching ? (
      ...
      {showReportModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          ...
          {isReportLoading ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 0', color: '#D4AF37' }}>
              ...
              <p style={{ marginTop: '10px', color: '#666', fontWeight: 'bold' }}>Loading today's sales...</p>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    ...
                    <td style={{ padding: '10px', border: '1px solid #ddd' }}>{(sale.dateObj || new Date(sale.date)).toLocaleTimeString()}</td>
